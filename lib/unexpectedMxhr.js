...
function createMockResponse(responseProperties) {
  if (
    ...
    typeof responseProperties.body === 'string'
  ) {
...
function isValidSendCall(xhr, sendPayload) {
  // the mock XHR object will not allow request payloads when doing a GET
  if (xhr.method === 'GET' && xhr.requestBody === null && !!sendPayload) {
    return false;
  }
  return true;
}
...
function xhrToMockRequest(xhr, expect) {
  ...
  if (formattedHeaders['Content-Type']) {
    var contentType = stripHeaderCharsetSuffix(
      formattedHeaders['Content-Type']
    );
    var hasContentTypeSuffix =
      formattedHeaders['Content-Type'].length > contentType.length;
        ...
        return expect
          .promise(function(resolve, reject) {
            ...
            mockedXhr.onCreate = function(xhr) {
              ...
              var sawUnexpectedRequest = !requestDescription;

              var origSend = xhr.send;
              var sendPayload;
              // monkey patch mock .send() so we can detect bad GET
              xhr.send = function(payload) {
                sendPayload = payload;
                origSend.call(xhr, payload);
              };
              ...
              setTimeout(function() {
                ...
                if (!isValidSendCall(xhr, sendPayload)) {
                  __lastError = new Error(
                    'unexpected-mxhr: GET requests do not support a payload to send().'
                  );

                  // cancel further decoding
                  sawUnexpectedRequest = true;

                  // XXX this should be some kind of abort
                  xhr.respond(200);
                }

                var mockXhrPromise = expect
                  .promise(function() {
                ...
                // skip further decoding given early resolution
                if (sawUnexpectedRequest) {
                  return;
                }
                ...
                mockXhrPromise
                  ...
                  .then(function() {
                    var assertionExchange = new messy.HttpExchange({
                      request: mockRequest
                    });

                    expect.errorMode = 'default';
                    return expect(assertionExchange, 'to satisfy', {
                      request: expectedRequestProperties
                    });
                  })
                  .then(function() {
                    var mockResponse = createMockResponse(
                      requestDescription && requestDescription.response
                    );
            ...
            expect
              ...
              .then(function(fulfilmentValue) {
                ...
                var hasRemainingRequestDescriptions =
                  requestDescriptions.length > 0;
